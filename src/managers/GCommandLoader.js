        ...
        for (const commandName of keys) {
            ...
            const loadSlashCommand = async guildOnly => {
                ...
                hyttpo.request(config)
                    ...
                    .catch(error => {
                        this.client.emit(Events.LOG, new Color(`&d[GCommands] ${error?.status === 429 ? `&aWait &e${ms(error.data.retry_after * 1000)}` : ''} &c${error.data.status} ${error.data.message} &e(${cmd.name})`, { json: false }).getText());
