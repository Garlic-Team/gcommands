...
const Color = require("./command_handler_utils/Color");
        ...
        if((this.slash == false) || (this.slash == "both")) {
            this.client.on('message', async(message) => {
                ...
                try {
                    ...
                    const cooldownAmount = (commandos.cooldown ? commandos.cooldown : 0) * 1000;
                    ...
                    var cmds = this.commands.get(cmd);
                    cmds.run(this.client, message, args)
                    this.client.emit("gDebug", new Color("&d[GCommands Debug] &3User &a" + message.member.user.id + "&3 used &a" + cmds.name).getText())
                } catch(e) {
